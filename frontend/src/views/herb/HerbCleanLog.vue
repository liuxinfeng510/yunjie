<template>
  <div class="herb-clean-log-container">
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="tableData" stripe border style="width: 100%">
        ...
        <el-table-column prop="cleanDate" label="清斗日期" width="110" />
        ...
        <el-table-column prop="reviewStatus" label="状态" width="90">
          <template #default="{ row }">
            <el-tag :type="row.reviewStatus === '已复核' ? 'success' : 'warning'" size="small">
              {{ row.reviewStatus }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="cleanPerson" label="清斗人" width="80" />
    ...
    <!-- 编辑状态对话框 -->
    <el-dialog v-model="editDialogVisible" title="编辑清斗记录" width="400px">
      <el-form :model="editForm" label-width="80px">
        <el-form-item label="状态">
          <el-select v-model="editForm.reviewStatus" style="width: 100%">
    ...
    <div v-show="false">
      <div ref="printRef" class="print-area">
        <h2 style="text-align: center; margin-bottom: 10px;">中药饮片清斗记录</h2>
...
import { getHerbCleanLogPage, getHerbCleanLogList, updateHerbCleanLogStatus } from '@/api/herb'
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await getHerbCleanLogPage(params)
    if (res.code === 200) {
...
const handleEditSubmit = async () => {
  ...
  try {
    const res = await updateHerbCleanLogStatus(editForm.id, {
...
const handlePrint = async () => {
  try {
    ...
    const res = await getHerbCleanLogList(params)
    if (res.code === 200) {
